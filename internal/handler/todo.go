...
func (t *todoHandler) Create(c echo.Context) error {
  ...
  case "high":
	...
	todo, err := t.service.Create(req.Task, priority)
	if err != nil {
...
// @Summary	Find all todos. SortBy takse priority as input
// @Tags		todos
// @Param		task	    query		string	false	"task"
...
func (t *todoHandler) FindAll(c echo.Context) error {
	...
	res, err := t.service.FindAll(task, status, sortBy)
	if err != nil {
		return c.JSON(http.StatusInternalServerError,
			ResponseError{Errors: []Error{{Code: errors.CodeInternalServerError, Message: err.Error()}}})
	}
