...
func TestTodoHandler_FindAll(t *testing.T) {
	...
	tests := []struct {
		name       string
		createData []string // List of task creation data
		queryParam string   // Query string for filtering (e.g., ?task=Task1)
		want       want
		wantErr    bool
	}{
		{
			name:       "fetch_filtered_by_task",
			createData: []string{`{"task":"Task1"}`},
			queryParam: "?task=Task1",
			want: want{
				StatusCode: http.StatusOK,
				Response:   []byte(`{"data":[{"Task":"Task1","Status":"created"}]}`),
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			// Create test data
			for _, data := range tt.createData {
				createTask(t, e, handler, data)
			}

			// Prepare the request with optional query params
			req := httptest.NewRequest(http.MethodGet, "/todos"+tt.queryParam, nil)
			req.Header.Set(echo.HeaderContentType, echo.MIMEApplicationJSON)
			rec := httptest.NewRecorder()
			c := e.NewContext(req, rec)
			c.SetPath("/todos")
