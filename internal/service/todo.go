...
type Todo interface {
	...
	FindAll(task, status, sotrBy string) ([]*model.Todo, error)
}
...
func (t *todo) FindAll(task, status, sortBy string) ([]*model.Todo, error) {
	todo, err := t.todoRepository.FindAll(task, status, sortBy)
	if err != nil {
		return nil, err
	}
	return todo, nil
